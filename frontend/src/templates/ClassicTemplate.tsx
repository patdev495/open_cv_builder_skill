  ...
  const renderExperienceItem = (exp: any, layout: 'timeline' | 'cards' | 'text', hiddenFields: string[]) => {
    ...
    return (
      <div key={exp.id} className="flex flex-col gap-0.5 break-inside-avoid">
        ...
        <p className="text-xs leading-relaxed text-slate-750 whitespace-pre-line mt-1 print:text-black">
          {parseFormatting(exp.description)}
        </p>
      </div>
    );
    ...
    if (layout === 'cards') {
      return (
        <div key={edu.id} className="p-3 bg-slate-50/50 dark:bg-slate-900/40 rounded-lg border border-slate-200 dark:border-slate-800 flex flex-col gap-1 break-inside-avoid">
          ...
          {edu.description && <p className="text-[10px] text-slate-550 dark:text-slate-400 mt-1">{parseFormatting(edu.description)}</p>}
        </div>
      );
    }
    ...
    if (layout === 'text') {
      return (
        <div key={edu.id} className="flex flex-col gap-0.5 text-xs break-inside-avoid">
          ...
          {edu.description && <p className="text-[10px] text-slate-550 dark:text-slate-400 mt-0.5">{parseFormatting(edu.description)}</p>}
        </div>
      );
    }
                        ...
                        if (sec === 'summary' && cvData.summary) {
                          ...
                          return (
                            <div key={sec} data-section="summary" className="flex flex-col gap-1.5 break-inside-avoid">
                              ...
                              <p className="text-xs leading-relaxed text-slate-700 dark:text-slate-300 italic text-justify font-serif">{parseFormatting(cvData.summary)}</p>
                            </div>
                          );
                        }
